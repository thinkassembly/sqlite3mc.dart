...
// @dart=2.12
library sqlite3mc_flutter_libs;
...
const _platform = MethodChannel('sqlite3mc_flutter_libs');
...
Future<void> applyWorkaroundToOpenSqlCipherOnOldAndroidVersions() async {
  ...
  try {
    DynamicLibrary.open('libsqlite3mc.so');
  } on ArgumentError {
...
DynamicLibrary openCipherOnAndroid() {
  try {
    return DynamicLibrary.open('libsqlite3mc.so');
  } catch (_) {
    ...
    return DynamicLibrary.open('/data/data/$appId/lib/libsqlite3mc.so');
  }
}
DynamicLibrary openSqlite3mcOnIOS() {
  try {

    return DynamicLibrary.open('sqlite3mc.framework/sqlite3mc');
  } catch (e) {
  print(e);
