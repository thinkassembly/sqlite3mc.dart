...
int dartvfs_sync(sqlite3_file *file, int flags) {
   return dartSync(((dart_vfs_file *)file)->zPath,flags);
...
int dartvfs_lock(sqlite3_file *file, int flags) {
  return dartLock(((dart_vfs_file *)file)->zPath, flags);
                     // databases
}

int dartvfs_unlock(sqlite3_file *file, int flags) {
  return dartUnlock(((dart_vfs_file *)file)->zPath, flags);
}
...
int sqlite3_os_init(void) {
 ...
 //
//
 ////   dartvfs_trace_log1("Second VFS -------------",NULL);
  //  dartvfs_trace_log1(vfs3->zName,NULL);

    sqlite3_vfs_register(&vfs, 1);
    sqlite3mc_vfs_create(vfs.zName,1);
    sqlite3_vfs* vfs2 =   sqlite3_vfs_find("multipleciphers-inner_dart_wasm_bridge");

  //  vfstrace_register("dart_wasm_bridge", "multipleciphers-inner_dart_wasm_bridge",
   //                 &dartvfs_trace_log1, NULL, 0);
#else
  sqlite3_vfs_register(&vfs, 1);
