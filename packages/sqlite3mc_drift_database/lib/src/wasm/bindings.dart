// ignore_for_file: avoid_dynamic_calls, constant_identifier_names, non_constant_identifier_names
import 'dart:async';
import 'dart:convert';
...
import 'package:sqlite3/src/common/constants.dart';
import 'package:sqlite3/src/common/impl/utils.dart';
...
BigInt WASI_RIGHTS_FD_WRITE = BigInt.from(64);
final _context = p.Context(style: p.Style.url, current: '/');

String drainWriter(String prev, String current) {
  String text = prev + current;
  while (text.contains('\n') == true) {
    var splitText = text.split('\n');
    print(splitText.first);
    text = splitText.sublist(1).join("\n");
  }
...
class WasmBindings {
  ...
  // ignore: close_sinks
  final StreamController<QualifiedSqliteUpdate> _updates = StreamController.broadcast();
  ...
  WasmBindings(this.instance, InjectedValues values)
      : memory = values.memory,
        _malloc = instance.functions['dart_sqlite3_malloc']!,
        _free = instance.functions['dart_sqlite3_free']!,
        _create_scalar = instance.functions['dart_sqlite3_create_scalar_function']!,
        _create_aggregate = instance.functions['dart_sqlite3_create_aggregate_function']!,
        _create_window = instance.functions['dart_sqlite3_create_window_function']!,
        ...
        _sqlite3_libversion_number = instance.functions['sqlite3_libversion_number']!,
        _sqlite3_open_v2 = instance.functions['sqlite3_open_v2']!,
        _sqlite3_close_v2 = instance.functions['sqlite3_close_v2']!,
        _sqlite3_extended_errcode = instance.functions['sqlite3_extended_errcode']!,
        _sqlite3_errmsg = instance.functions['sqlite3_errmsg']!,
        _sqlite3_errstr = instance.functions['sqlite3_errstr']!,
        _sqlite3_extended_result_codes = instance.functions['sqlite3_extended_result_codes']!,
        ...
        _sqlite3_bind_parameter_count = instance.functions['sqlite3_bind_parameter_count']!,
        ...
        _sqlite3_bind_parameter_index = instance.functions['sqlite3_bind_parameter_index']!,
        _sqlite3_changes = instance.functions['sqlite3_changes']!,
        _sqlite3_last_insert_rowid = instance.functions['sqlite3_last_insert_rowid']!,
        ...
        _sqlite3_aggregate_context = instance.functions['sqlite3_aggregate_context']!,
        _sqlite3_temp_directory = instance.globals['sqlite3_temp_directory']! {
    values.bindings = this;
    functions = values.functions;
  }

  static Future<WasmBindings> instantiateAsync(Uint8List source, SqliteEnvironment environment) async {
    final injected = InjectedValues(environment);
  ...
  int create_scalar_function(Pointer db, Pointer functionName, int nArg, int eTextRep, int id) {
    return _create_scalar(db, functionName, nArg, eTextRep, id) as int;
  }

  int create_aggregate_function(Pointer db, Pointer functionName, int nArg, int eTextRep, int id) {
    return _create_aggregate(db, functionName, nArg, eTextRep, id) as int;
  }

  int create_window_function(Pointer db, Pointer functionName, int nArg, int eTextRep, int id) {
  ...
  int sqlite3_extended_errcode(Pointer db) => _sqlite3_extended_errcode(db) as int;
  ...
  int sqlite3_exec(Pointer db, Pointer sql, Pointer callback, Pointer callbackArg, Pointer errorOut) {
    return _sqlite3_exec(db, sql, callback, callbackArg, errorOut) as int;
  }

  int sqlite3_prepare_v3(Pointer db, Pointer sql, int length, int prepFlags, Pointer ppStmt, Pointer pzTail) {
    return _sqlite3_prepare_v3(db, sql, length, prepFlags, ppStmt, pzTail) as int;
  }
  ...
  int sqlite3_bind_int64(Pointer stmt, int index, BigInt value) {
    return _sqlite3_bind_int64(stmt, index, JsBigInt.fromBigInt(value.checkRange).jsObject) as int;
  }
  ...
  int sqlite3_bind_text(Pointer stmt, int index, Pointer text, int length, Pointer a) {
    return _sqlite3_bind_text(stmt, index, text, length, a) as int;
  }

  int sqlite3_bind_blob64(Pointer stmt, int index, Pointer test, int length, Pointer a) {
    return _sqlite3_bind_blob64(stmt, index, test, JsBigInt.fromInt(length).jsObject, a) as int;
  }
  ...
  void sqlite3_result_int64(Pointer context, BigInt value) {
    _sqlite3_result_int64(context, JsBigInt.fromBigInt(value.checkRange).jsObject);
  }
  ...
  void sqlite3_result_text(Pointer context, Pointer text, int length, Pointer a) {
    _sqlite3_result_text(context, text, length, a);
  }

  void sqlite3_result_blob64(Pointer context, Pointer blob, int length, Pointer a) {
  ...
  int sqlite3_last_insert_rowid(Pointer db) => JsBigInt(_sqlite3_last_insert_rowid(db) as Object).asDartInt;
...
class InjectedValues {
  ...
  InjectedValues(SqliteEnvironment environment) {
    ...
    injectedValues = {
      ...
      'wasi_snapshot_preview1': {
        // provide random_get implementation for wasi
        'random_get': allowInterop((int ptr, int length) {
          final buffer = memory.buffer.asUint8List(ptr, length);

          /// The `crypto` object.

          final Crypto? crypto = getProperty(globalThis, 'crypto');
        ...
        'fd_close': allowInterop((int ptr) {
          return 0;
        }),
        'fd_fdstat_get': allowInterop((int fd, int stat) {
          if (fd != 1 && fd != 2) {
            return WASI_ERRNO_BADF;
          }

          final view = ByteData.view(this.memory.buffer);
          view.setUint8(stat + 0, WASI_FILETYPE_CHARACTER_DEVICE);
          view.setUint32(stat + 2, WASI_FDFLAGS_APPEND, Endian.little);

          view.setInt32(stat + 8, WASI_RIGHTS_FD_WRITE.toInt(), Endian.little);
          //view.setInt64(stat + 8, WASI_RIGHTS_FD_WRITE.toInt());

          view.setInt32(stat + 16, WASI_RIGHTS_FD_WRITE.toInt(), Endian.little);
        ...
        'fd_seek': allowInterop((int ptr, int offset_low, int offset_high, int whence, int newOffset) {
          return 0;
        }),
        'fd_write': allowInterop((int fd, int iovs, int iovsLen, int nwritten) {
          if (fd != 1 && fd != 2) {
            return WASI_ERRNO_BADF;
          }
          final view = ByteData.view(this.memory.buffer);
          ...
          for (var i = 0; i < iovsLen; i++) {
            var ptr = iovs + i * 8;
            var buf = view.getUint32(ptr, Endian.little);
            var bufLen = view.getUint32(ptr + 4, Endian.little);
            buffers.add(this.memory.buffer.asUint8List(buf, bufLen));
          }
          var written = 0;
          var text = '';
          for (var element in buffers) {
            text += utf8.decode(element);
            written += element.lengthInBytes;
          }

          view.setUint32(nwritten, written, Endian.little);
          if (fd == STDOUT) {
            stdoutText = drainWriter(stdoutText, text);
          } else if (fd == STDERR) {
            stderrText = drainWriter(stderrText, text);
          }

          //      var buffers = List<Uint8List>.from( this.memory.buffer.asUint8List(), );)
          return WASI_ESUCCESS;
        }),
      },
      'dart': {
        ...
        'path_normalize': allowInterop((Pointer source, Pointer dest, int length) {
        ...
        'function_xInverse': allowInterop((Pointer ctx, int args, Pointer value) {
        ...
        'function_hook': allowInterop((int id, int kind, Pointer _, Pointer table, Object rowId) {
          ...
          bindings._updates.add(QualifiedSqliteUpdate(id, SqliteUpdate(kindEnum, tableName, JsBigInt(rowId).asDartInt)));
        }),
        'fs_sync': allowInterop((Pointer path, int flags) {
          try {
            final pathStr = memory.readString(path);
            return environment.fileSystem.fsync(pathStr, flags);
          } catch (e) {
            return 1;
          }
        }),
        'fs_unlock': allowInterop((Pointer path, int flags) {
          try {
            final pathStr = memory.readString(path);
            return environment.fileSystem.unlock(pathStr, flags);
          } catch (e) {
            print(e);
          }
        }),
        'fs_lock': allowInterop((Pointer path, int flags) {
          try {
            final pathStr = memory.readString(path);
            return environment.fileSystem.lock(pathStr, flags);
          } catch (e) {
            print(e);
          }
        }),
        ...
        'fs_size': allowInterop((Pointer path, Pointer pSize) {
          try {
            final size = environment.fileSystem.sizeOfFile(memory.readString(path));
        ...
        'fs_read': allowInterop((Pointer path, Pointer into, int amount, Object offset) {
          try {
            return environment.fileSystem
                .read(memory.readString(path), memory.buffer.asUint8List(into, amount), JsBigInt(offset).asDartInt);
          } on FileSystemException catch (e) {
            return -e.errorCode;
          } catch (e) {
        ...
        'fs_write': allowInterop((Pointer path, Pointer from, int amount, Object offset) {
          try {
            environment.fileSystem.write(memory.readString(path), memory.buffer.asUint8List(from, amount), JsBigInt(offset).asDartInt);
            return 0;
          } on FileSystemException catch (e) {
            return e.errorCode;
          }
        ...
        'fs_access': allowInterop((Pointer path, int flags, Pointer pResOut) {
          try {
            final exists = environment.fileSystem.exists(memory.readString(path));
